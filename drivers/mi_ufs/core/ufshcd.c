...
#include "../include/ufs_quirks.h"
#include "../include/unipro.h"
#include "ufs-sysfs.h"
...
#endif
#define DFX_ID_MEMORY_FAIL 910013002
...
#define QUERY_REQ_TIMEOUT 5500 /* 5.5 seconds */

/* Advanced RPMB request timeout */
#define ADVANCED_RPMB_REQ_TIMEOUT  3000 /* 3 seconds */

/* Task management command timeout */
#define TM_CMD_TIMEOUT	30000 /* msecs */
...
#if IS_ENABLED(CONFIG_UFS_FFU)
DECLARE_COMPLETION(ufs_xiaomi_comp);
EXPORT_SYMBOL(ufs_xiaomi_comp);
#endif
...
void mievent_upload(int miev_code, char *err_key, char *err_value)
{
#if (IS_ENABLED(CONFIG_MIEV))
	if (err_key != NULL && err_value != NULL) {
		struct misight_mievent *event = cdev_tevent_alloc(miev_code);
		if (cdev_tevent_add_str(event, err_key, err_value) != -1) {
...
{
       int offset = 9;
       struct UFS_DATA *ufs_data_ptr = &ufs_data;
       ...
       memcpy(ufs_data_ptr->ufs_err_state.err_reason, err_reason, strlen(err_reason) + 1);
       dump_stack();
}
...
int mi_scsi_execute_cmd(struct scsi_device *sdev, const unsigned char *cmd,
...
EXPORT_SYMBOL(mi_scsi_execute_cmd);
...
{
	up_write(&hba->clk_scaling_lock);
//	dev_info(hba->dev, "%s keep write booster always on\n", __func__);
#if 0
...
ufshcd_wait_for_uic_cmd(struct ufs_hba *hba, struct uic_command *uic_cmd)
{
	...
	if ((ret) && (ret != -ETIMEDOUT)) {
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "uic");
	}
...
static int ufshcd_queuecommand(struct Scsi_Host *host, struct scsi_cmnd *cmd)
{
	...
	struct ufshcd_lrb *lrbp;
	int err = 0;
#ifdef CONFIG_UFS_SWAP
	u64 lba = 0;
	struct ufsswap_lba_info *lba_info_tmp;
	struct list_head *head = &hba->swap_ctrl.lba_list_head;
...
#ifdef CONFIG_UFS_SWAP
	lba = scsi_get_lba(cmd);
	if (hba->swap_ctrl.swap_enable && !list_empty(head)) {
		list_for_each_entry(lba_info_tmp, head, list) {
			if (lba >= lba_info_tmp->lba_pre && lba <= lba_info_tmp->lba_post){
				if (cmd->cmnd[0] == 0x2A)
				{
					cmd->cmnd[6] = swap_info->swap_dev_spec->gr_write;
					break;
				}
				if (cmd->cmnd[0] == 0x28)
				{
					cmd->cmnd[6] = swap_info->swap_dev_spec->gr_read;
					break;
				}
				break;
			}
...
retry:
	...
	if ((err) && (err != -EAGAIN)) {
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "dev");
	}
...
int __ufshcd_query_descriptor(struct ufs_hba *hba,
...
/**
 * mi_ufshcd_query_descriptor_retry - API function for sending descriptor requests
 * @hba: per-adapter instance
 * @opcode: attribute opcode
 * @idn: attribute idn to access
 * @index: index field
...
/**
 * mi_ufshcd_read_desc_param - read the specified descriptor parameter
 * @hba: Pointer to adapter instance
...
out:
	if (ret) {
		ufsdbg_set_err_state("uic_pwr_ctrl_fail\n");
...
int ufshcd_uic_hibern8_enter(struct ufs_hba *hba)
{
	...
	if (ret){
		ufsdbg_set_err_state("hibern8_enter_fail\n");
		dev_err(hba->dev, "%s: hibern8 enter failed. ret = %d\n",
			__func__, ret);
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "h8_enter");
	} else
...
int ufshcd_uic_hibern8_exit(struct ufs_hba *hba)
{
	...
	if (ret) {
		...
		dev_err(hba->dev, "%s: hibern8 exit failed. ret = %d\n",
			__func__, ret);
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "h8_exit");
	} else {
			     ...
			     struct ufs_pa_layer_attr *pwr_mode)
{
	...
	if (ret) {
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "pow_mod_chg");
	} else {
...
link_startup:
	do {
		...
		if (ret) {
			...
			mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "link_startup");
		}
		...
		if (!ret && !ufshcd_is_device_present(hba)) {
			...
			mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "dev_no_pre");
			ret = -ENXIO;
			goto out;
		}
...
static void ufshcd_eua_configure(struct ufs_hba *hba, struct scsi_device *sdev)
{
#ifdef CONFIG_UFS_EUA
	if (sdev->lun == 0)
		hba->eua_ctrl.sdev_ufs_lu = sdev;
#endif
	return;
}
			   ...
			   struct cq_entry *cqe)
{
	...
	if ((host_byte(result) == DID_ERROR) ||
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "tran_rsp");
	}
	return result;
}
...
int ufshcd_wb_toggle(struct ufs_hba *hba, bool enable)
{
	...
	hba->dev_info.wb_enabled = enable;
	dev_err(hba->dev, "%s: ll2 Write Booster %s\n",
			__func__, enable ? "enabled" : "disabled");

	return ret;
}

void ufshcd_wb_toggle_buf_flush_during_h8(struct ufs_hba *hba,
						 bool enable)
{
	int ret;
...
static void ufshcd_err_handler(struct work_struct *work)
{
	...
	mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "err_handler");
...
static int ufshcd_abort(struct scsi_cmnd *cmd)
{
	...
	mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "abort");
...
#ifdef CONFIG_UFS_EBUFF
	if (ebuff_value_u32("device_quirks", &value)) {
		u32 mask = 0;
		if (ebuff_value_u32("dev_quirks_mask", &mask)) {
			dev_info(hba->dev,"%s: ebuff origin dev_quirks 0x%x value 0x%x mask 0x%x\n",
				__func__, hba->dev_quirks, value, mask);
			hba->dev_quirks = (hba->dev_quirks & (~mask)) |(value & mask);
		} else {
			dev_info(hba->dev,"%s: ebuff origin dev_quirks 0x%x value 0x%x\n",
				__func__, hba->dev_quirks, value);
			hba->dev_quirks |= value;
		}
	}
#endif
}
...
static void ufshcd_tune_host_synclength_config(struct ufs_hba *hba)
{
#ifdef CONFIG_UFS_EBUFF
	u32 value = 0;
	int ret = 0;
	if (ebuff_value_u32("hsg1_synclength_host", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG1SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set host hsg1_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg2_synclength_host", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG2SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set host hsg2_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg3_synclength_host", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG3SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set host hsg3_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg4_synclength_host", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG4SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set host hsg4_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg5_synclength_host", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TXHSG5SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set host hsg5_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
#endif
	return;
...
static void ufshcd_tune_device_synclength_config(struct ufs_hba *hba)
{
#ifdef CONFIG_UFS_EBUFF
	u32 value = 0;
	int ret = 0;
	if (ebuff_value_u32("hsg1_synclength_device", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG1SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set device hsg1_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg2_synclength_device", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG2SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set device hsg2_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg3_synclength_device", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG3SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set device hsg3_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg4_synclength_device", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG4SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set device hsg4_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
	if (ebuff_value_u32("hsg5_synclength_device", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG5SYNCLENGTH),value);
		dev_info(hba->dev,"%s:ebuff set device hsg5_synclength value 0x%x,ret=%d\n",
			__func__, value, ret);
	}
#endif
	return;
...
static void ufshcd_tune_tactivate_hibern8time_config(struct ufs_hba *hba)
{
#ifdef CONFIG_UFS_EBUFF
	u32 value = 0;
	int ret = 0;
	if (ebuff_value_u32("tActive_host", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_TACTIVATE), value);
		dev_info(hba->dev,"%s:ebuff set tactive host value %d us,ret=%d\n",
			 __func__, value * 100, ret);
	}
	if (ebuff_value_u32("tActive_device", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TACTIVATE),value);
		dev_info(hba->dev,"%s:ebuff set tactive device value %d us,ret=%d\n",
                         __func__, value * 100, ret);
	}
	if (ebuff_value_u32("tHibernate8", &value)) {
		ret = ufshcd_dme_set(hba, UIC_ARG_MIB(PA_HIBERN8TIME), value);
		dev_info(hba->dev,"%s:ebuff set tHibernate8 value %d us,ret=%d\n",
			 __func__, value * 100, ret);
	}
#endif
	return;
}
static void ufshcd_tune_de_emphasis_config(struct ufs_hba *hba)
{
#ifdef CONFIG_UFS_EBUFF
	u32 value = 0;
	int ret = 0;
	if (ebuff_value_u32("de-emphasis", &value)) {
		ret = ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSEQUALIZER_SETTING), value);
		dev_info(hba->dev, "%s:ebuff set de-emphasis value %d,ret=%d\n",
			 __func__, value, ret);
	}
#endif
	return;
...
static void ufshcd_tune_unipro_params(struct ufs_hba *hba)
{
	...
	if (dev_info->wspecversion > 0x0310)
		ufshcd_dme_peer_set(hba, UIC_ARG_MIB(PA_TXHSG5SYNCLENGTH), 0x4f);
	ufshcd_tune_host_synclength_config(hba);
	ufshcd_tune_device_synclength_config(hba);
	ufshcd_tune_tactivate_hibern8time_config(hba);
	ufshcd_tune_de_emphasis_config(hba);
}
...
#if IS_ENABLED(CONFIG_UFS_FFU)
	dev_err(hba->dev, "%s: increase ufs_xiaomi_comp\n",
					__func__);
	complete(&ufs_xiaomi_comp);
	dev_err(hba->dev, "%s: increase ufs_xiaomi_comp end\n",
					__func__);
#endif
	...
	list_for_each_entry(clki, head, list) {
		if (!IS_ERR_OR_NULL(clki->clk)) {
			...
			if (ebuff_value_u32("ref_clk_always_on", &value)) {
				if (!strncmp(clki->name, "ref_clk",strlen("ref_clk"))) {
...
{
	//	dev_info(hba->dev, "%s keep vcc/vccq always on + npm \n", __func__);
	return;
	/*keep vcc/vccq always on +npm*/
#if 0
	bool vcc_off = false;
	/*
...
set_link_active:
	...
	if (ufshcd_is_link_hibern8(hba) && !ufshcd_uic_hibern8_exit(hba))
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "link_off");
...
out:
	...
	if (ret) {
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "suspend");
...
out:
	if (ret) {
		...
		mievent_upload(DFX_ID_MEMORY_FAIL, "errInfo", "resume");
...
int ufshcd_core_init(struct ufs_hba *hba)
{
	int ret;
