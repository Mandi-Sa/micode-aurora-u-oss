...
int sk_cld_set_trigger(struct ufscld_dev *cld, u32 trigger)
{
	...
	return ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_WRITE_ATTR, (enum attr_idn)QUERY_ATTR_IDN_CLD_ENABLE, 0, 0, &trigger);
}
...
int sk_cld_get_trigger(struct ufscld_dev *cld, u32 *trigger)
{
	...
	return ufshcd_query_attr_retry(hba, UPIU_QUERY_OPCODE_READ_ATTR, (enum attr_idn)QUERY_ATTR_IDN_CLD_ENABLE, 0, 0, trigger);
}
