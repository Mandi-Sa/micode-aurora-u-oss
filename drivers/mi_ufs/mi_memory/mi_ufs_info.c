...
static ssize_t dump_health_desc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = ufshcd_read_desc(hba, QUERY_DESC_IDN_HEALTH, 0, desc_buf, buff_len);
	if (ret) {
		dev_err(hba->dev, "%s: Failed reading  dump_health_desc_show, ret %d\n",__func__,  ret);
		kfree(desc_buf);
		return count;
	}
...
*/
	count += snprintf((buf + count), PAGE_SIZE, "%s", ser_number);
